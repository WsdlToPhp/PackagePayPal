...
abstract class AbstractResponseType extends AbstractStructBase
{
    ...
    public function setAck($ack = null)
    {
        ...
        if (!\PayPal\EnumType\AckCodeType::valueIsValid($ack)) {
            throw new \InvalidArgumentException(sprintf('Value "%s" is invalid, please use one of: %s', $ack, implode(', ', \PayPal\EnumType\AckCodeType::getValidValues())), __LINE__);
        }
        $this->Ack = $ack;
        return $this;
    }
    ...
    public function setErrors(array $errors = array())
    {
        foreach ($errors as $abstractResponseTypeErrorsItem) {
            ...
            if (!$abstractResponseTypeErrorsItem instanceof \PayPal\StructType\ErrorType) {
                throw new \InvalidArgumentException(sprintf('The Errors property can only contain items of \PayPal\StructType\ErrorType, "%s" given', is_object($abstractResponseTypeErrorsItem) ? get_class($abstractResponseTypeErrorsItem) : gettype($abstractResponseTypeErrorsItem)), __LINE__);
            }
        }
        $this->Errors = $errors;
        return $this;
    ...
    public function addToErrors(\PayPal\StructType\ErrorType $item)
    {
        ...
        if (!$item instanceof \PayPal\StructType\ErrorType) {
            throw new \InvalidArgumentException(sprintf('The Errors property can only contain items of \PayPal\StructType\ErrorType, "%s" given', is_object($item) ? get_class($item) : gettype($item)), __LINE__);
        }
        $this->Errors[] = $item;
        return $this;
    }
     ...
     * @uses \DOMNode::item()
     * @uses \PayPal\StructType\AbstractResponseType::setAny()
